...
require_once __DIR__ . '/../backend/cloudinary.php';

$localUploadsDir = __DIR__ . '/../photo/uploads';

// Create connection to admin database
$conn = create_db_connection(get_database_name('admins'));
...
if (isset($_GET["delete"])) {
    ...
    if ($result->num_rows > 0) {
        ...
        if (!empty($row['image_path']) && strpos($row['image_path'], '/photo/uploads/') === 0) {
            $localFile = __DIR__ . '/..' . $row['image_path'];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($errors)) {
        ...
        if (isset($_FILES['photo']) && $_FILES['photo']['error'] == 0) {
            ...
            // Prefer Cloudinary when configured; fallback to local uploads.
            if (is_cloudinary_configured()) {
                $cloudinaryUpload = upload_image_to_cloudinary($tmpFile);
                if ($cloudinaryUpload['success']) {
                    $imagePath = $cloudinaryUpload['url'];
                } else {
                    $errors[] = "Image upload failed: " . $cloudinaryUpload['error'];
                }
            } else {
                $fileName = uniqid() . "." . $ext;
                $localFilePath = $localUploadsDir . "/" . $fileName;
                $imagePath = "/photo/uploads/" . $fileName;
                move_uploaded_file($tmpFile, $localFilePath);
            }
        }
        ...
        if (!empty($errors)) {
            ...
            if (isset($_POST['edit_id'])) {
                ...
                $notification = "Ã¢Å“ÂÃ¯Â¸Â Listing updated successfully!";
            } else {
                // Insert new record
                ...
                $notification = "Ã¢Å“â€¦ Thank you, $seller_name! Your product has been listed.";
            }
            ...
            exit;
                ...
                $notification = "Ã¢Å¡Â Ã¯Â¸Â Error: " . $e->getMessage();
            }
        }
    }
}
...
<body>
    <div class="main-container">
        <div class="background-image">
            <img src="../photo/imgae.png" alt="Background">
        </div>
        <div class="content-wrapper">
            ...
            <form method="POST" enctype="multipart/form-data" class="bg-white bg-opacity-90 p-8 rounded-xl shadow-md mb-10">
                <h2 class="text-2xl font-bold text-blue-600 mb-6">
                    <?= $editMode ? "Ã¢Å“ÂÃ¯Â¸Â Edit Your Listing" : "Ã°Å¸â€ºÂÃ¯Â¸Â Sell Your Product" ?>
                </h2>
                ...
                <div class="my-6">
                    <label class="block font-semibold mb-2 text-blue-700">
                        Ã°Å¸â€œÂ Your Location on Map (search your city or area):
                    </label>
            ...
            <div class="bg-white bg-opacity-90 p-6 rounded-xl shadow-md">
                <h3 class="text-xl font-semibold mb-4 text-blue-700">Ã°Å¸â€œâ€¹ Submitted Products</h3>
                                ...
                                <tr>
                                    <td class="p-3 border">
                                        <?php if ($submission['image_path']): ?>
                                            <img src="<?= htmlspecialchars(strpos($submission['image_path'], 'http') === 0 ? $submission['image_path'] : '..' . $submission['image_path']) ?>" alt="Product Photo" class="w-16 h-16 object-cover">
                                        <?php endif; ?>
                                    </td>
                                    ...
                                    <td class="p-3 border">
                                        <a href="?edit=<?= $submission['id'] ?>" class="text-yellow-600 hover:text-yellow-800 mr-3">Ã¢Å“ÂÃ¯Â¸Â Edit</a>
                                        <a href="?delete=<?= $submission['id'] ?>" class="text-red-600 hover:text-red-800">Ã°Å¸â€”â€˜Ã¯Â¸Â Delete</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
